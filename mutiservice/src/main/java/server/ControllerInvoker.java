...
import server.route.RouteResult;
import server.util.TypeConvertUtils;
...
public class ControllerInvoker {
    ...
    public void invoke(String method, String uri, WebContext webContext) {
        ...
        try {
            ...
            Object controllerObject = this.objectFactory.instance(handleMethod.getDeclaringClass().getCanonicalName(),
                    true);
            ...
            if (returnObj != null) {
                webContext.getResponse().setContentType(
                        String.format("application/json; charset=%s", serverConfig.getCharset()));
                webContext.getResponse().writeBody(JsonUtil.OBJECT_MAPPER.writeValueAsString(returnObj));
            }

        } catch (Exception e) {
            exception = e;
            throw new WebException("Invoke method fail :" + routeResult.getRouteInfo().getHandleMethod() + ":"
